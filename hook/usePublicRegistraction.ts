...
export function usePublicRegistrationPanel({
  ...
  const filtered = useMemo(() => {
    return data.filter((r) => {
      ...
      const matchesFilter = filter === "All" || r.state === filter
  ...
  const acceptRequest = async () => {
    if (!selected) return
    const promise = acceptRequestAction({ id: selected.id, customMessage: message || undefined })
  ...
  const rejectRequest = async () => {
    if (!selected) return
    const promise = rejectRequestAction({ id: selected.id, customMessage: message || undefined })
    toast.promise(promise, {
      loading: "Processing...",
