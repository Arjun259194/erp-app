...
export class MailBuilder<T extends Partial<BuilderInputs>> {
  #actual: T;
  ...
  from(from: string) {
    return new MailBuilder({ ...this.#actual, from });
  }
  to(to: string) {
    return new MailBuilder({ ...this.#actual, to });
  }
...
export default class SMTPGmailService {
  ...
  public static getInstance(config: MailerCred): SMTPGmailService {
