...
const SalesSchema = z.object({
  ...
  overDeliveryAllowance: z
    ...
    .refine(
      ...
      {
        message:
          "Over delivery allowance must be between 0-100%",
      },
    ),
  overBillingAllowance: z
    ...
    .refine(
      ...
      {
        message:
          "Over billing allowance must be between 0-100%",
      },
    ),
})
  ...
  try {
    await DB.UpdateItemById(parsed.id, {
      allowAlternativeItem:
        parsed.allowAlternativeItem === "on",
      overDeliveryAllowance:
        parsed.overDeliveryAllowance,
      overBillingAllowance:
        parsed.overBillingAllowance,
    })
  } catch (error) {
    console.log(error)
