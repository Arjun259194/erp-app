...
export const handleLogin: ServerAction<{
  name: string
}> = async formdata => {
  "use server"
  ...
  if (user.status === "Suspended")
    throw new Error(
      "You have bees suspended by admin",
    )
...
export const handleForgotPass: ServerAction =
  async formdata => {
    const safeEmail = z
      .string()
      .email()
    ...
    if (!safeEmail.success)
      throw new Error(
        safeEmail.error.issues[0].message,
      )
    ...
    if (!user)
      throw new Error(
        "Email doesn't exists, check credentials",
      )
    if (user.status === "Suspended")
      throw new Error(
        "You have bees suspended by admin",
    ...
    const recent =
      await DB.FindLatestForgotPasswordRequest(
        user.id,
      )
    if (recent)
      throw new Error(
    ...
    const mailer =
      SMTPGmailService.getInstance(defMailCred)
    const jwtoken = JWToken.getInstance()
    ...
    const token = jwtoken.serialize(
      payload,
      "5 Minutes",
    )

    try {
      await DB.CreateNewForgotPasswordRequest(
        user.id,
      )
    } catch (err) {
      console.error(err)
...
export const handleLoginWithEmail: ServerAction =
  async formdata => {
    const safeEmail = z
      .string()
      .email()
    ...
    const recent =
      await DB.FindLatestEmailLoginRequest(
        user.id,
      )
