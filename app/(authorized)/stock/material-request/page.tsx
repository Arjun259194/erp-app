...
export default async function page() {
  ...
  if (error)
    return gotError("Authentication Error", error)
  ...
  if (!activeCompanyId && user.role !== "Admin") {
    throw new Error(
      "No active company found for the user.",
    )
  }
  const x = userCompanies.map(x => {
    return x.companyId
  })
  // Fetch material requests with all required includes
  const materialRequests =
    await DB.GetAllMaterialRequests() //TODO add company id for filtering
