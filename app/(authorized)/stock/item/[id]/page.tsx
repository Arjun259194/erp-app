...
export default async function EditItemPage({ params }: { params: Promise<{ id: string }> }) {
  const item = await DB.GetItemById((await params).id);
  if (!item) notFound();
  ...
  return (
    <Tabs defaultValue="basic" className="space-y-8">
      <TabsList>
        ...
        <TabsTrigger value="sales">Sales &amp; Delivery</TabsTrigger>
        <TabsTrigger value="tax">Tax/Finance</TabsTrigger>
        <TabsTrigger value="manufacturing">Manufacturing</TabsTrigger>
        <TabsTrigger value="meta">System</TabsTrigger>
      </TabsList>
      ...
      <TabsContent value="basic">
        <form action={basicAction}>
          ...
          <BasicInfo item={item} itemGroups={itemGroups} />
          <SaveBar />
        </form>
      </TabsContent>
      ...
      <TabsContent value="inventory">
        <form action={inventoryAction}>
          ...
          <input type="hidden" name="id" value={item.id} /> {/* ← Add this line */}
          <Inventory item={item} />
          <SaveBar />
        </form>
      </TabsContent>
      ...
      <TabsContent value="sales">
        <form action={salesAction}>
          <h2 className="text-lg font-semibold mb-4">Sales &amp; Delivery</h2>
          <input type="hidden" name="id" value={item.id} /> <SalesDelivery item={item} />
          <SaveBar />
        </form>
      </TabsContent>
      ...
      <TabsContent value="tax">
        <form action={taxAction}>
          <h2 className="text-lg font-semibold mb-4">Tax &amp; Finance</h2>
          <input type="hidden" name="id" value={item.id} /> <TaxFinance item={item} />
          <SaveBar />
        </form>
      </TabsContent>
      ...
      <TabsContent value="manufacturing">
        <form action={mfgAction}>
          <h2 className="text-lg font-semibold mb-4">Manufacturing</h2>
          <input type="hidden" name="id" value={item.id} /> <Manufacturing item={item} />
          <SaveBar />
        </form>
      </TabsContent>
      ...
      <TabsContent value="meta">
        <form action={metaAction}>
          <h2 className="text-lg font-semibold mb-4">System Metadata</h2>
          <input type="hidden" name="id" value={item.id} /> <SystemMeta item={item} />
          <SaveBar />
        </form>
      </TabsContent>
    </Tabs>
